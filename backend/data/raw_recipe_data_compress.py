...
STOPWORDS = {
    "fresh", "freshly", "large", "small", "medium", "chopped", 
    "diced", "minced", "to", "taste", "and", "or", "optional", 
    "temperature", "sliced", "grated", "shredded", "only", 
    "weight", "garnish", "dough"
}

UNIT_WORDS = {
    "cup", "cups", "tbsp", "tablespoon", "tablespoons",
    "tsp", "teaspoon", "teaspoons", "oz", "ounce", 
    "ounces", "lb", "pound", "pounds", "grams", "g", "kg", 
    "ml", "l", "liter", "liters", "pinch", "clove", "cloves", 
    "can", "cans", "handful", "bunch", "bunches", "piece", 
    "pieces", "seeds", "head", "heads", "slice", "slices", 
    "strip", "strips", "half", "halves", "quarter", "quarters", 
    "cubes", "pack", "packs", "packet", "stick", "sticks", 
    "bag", "bags", "box", "boxes", "leaf", "leaves", "granules"
}

SYNONYMS = [
    (r"^(extra_virgin|extravirgin|evoo)(_olive(_oil)?)?$", "extra_virgin_olive_oil"),
    (r"^(?:ground_|cracked_|coarse_)?(?:black_)?pepper$", "black_pepper"),
    (r"^confectioners(_sugar)?$", "confectioners_sugar"),
    (r"^vanilla_(essence|extract)$", "vanilla_extract"),
]

def synonym_mapping(norm: str) -> str:
...
def normalize_ingredient(text: str):
    ...
    norm = "_".join(tokens[:3])  # keep core ingredient phrase
    norm = synonym_mapping(norm)
    return norm
